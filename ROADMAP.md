...
- [x] Publish an interactive HTML preview to GitHub Pages
...
- 2026-08-22 — Added `docs/index.html`: a self-contained, dependency-free GitHub Pages landing/preview (served from `/docs`) — hero, sample-findings stat cards, an interactive tabbed chart viewer embedding the three verified PNGs from `docs/charts/`, the architecture flow, and links to the ERD and data dictionary. Data-independent (reuses already-generated sample charts). Verified: HTML parses with balanced tags via `html.parser`, and every local `src`/`href` in the file resolves (3 chart PNGs + ERD.md + DATA_DICTIONARY.md all present). (First two roadmap items remain blocked here: `make full` needs a local internet-connected machine, and the CPI deflator needs a real CPI series plus a `fact_cpi` table not yet in the schema — CSO API unreachable from the sandbox; the `.pbix` build needs Power BI Desktop.)
